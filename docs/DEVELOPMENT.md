...
This covers everything needed to run SnaitySoft Polls from source, package a release build,
and understand how releases are cut. For how to propose changes (branches, commit style, PRs),
...
- Tauri's [platform-specific dependencies](https://v2.tauri.app/start/prerequisites/) (on
  Windows this is mainly the WebView2 runtime, usually already present)
...
YouTube needs no credentials — see [Platform integrations](#platform-integrations) below for why.
...
[Secrets & the encrypted `.env`](#secrets--the-encrypted-env) below for why. This adds one step
before packaging:
...
- `src/components/` — React components, grouped by feature area (poll, chat, connections,
  settings, overlay).
- `src/lib/` — framework-agnostic logic: chat clients, poll state helpers, i18n dictionaries,
  auth, logging.
- `src/store/` — Zustand stores.
- `src-tauri/` — the Rust/Tauri backend: Axum server (overlay page, poll WebSocket, OAuth
  callbacks), OAuth flows, `.env` encryption.
